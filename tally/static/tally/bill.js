...
document.addEventListener('DOMContentLoaded', function() {   
   ...
   // check if bill is loaded
    if (document.getElementById("bill") != undefined){         
        ...
        // Get the bill currency symbol 
        currency = document.getElementById('bill_symbol').innerHTML 
        ...
        document.getElementById('add_line').addEventListener('click', () => { 
            ...
            document.getElementById('id_discount').value = document.getElementById('bill_discount').innerHTML;                        
            $('#line_modal').modal('show');            
        });

         
        // add an event listener to the item form submit 
        document.getElementById('line_form_modal').addEventListener('submit', () => {
        ...
        document.getElementById('user_form_modal').addEventListener('submit', () => {
        ...
        document.getElementById('btn_user_del').addEventListener('click', ()=> {
            ...
            document.getElementById('delete_modal_id').innerHTML = document.getElementById('id_user').value;
            document.getElementById('delete_modal_type').innerHTML = 'user';
            document.getElementById('delete-modal-text').innerHTML = 'Are you sure you want to delete the person?';
            $('#user_modal').modal('hide');
        ...
        document.getElementById('show_bill_details').addEventListener('click',()=>{
    ...
    if   (document.getElementById('bill_id')){ 
        ...
        const mutationObserverLineItem = new MutationObserver(() =>{           
            ...
            Array.from(table.rows).forEach((row)=>{
                if (row.cells[2]){
                    sum_value += Number((row.cells[2].innerHTML).replace(currency,''));           
                }
            })    
            document.getElementById('total_bill').innerHTML = 'Total: ' + currency + Intl.NumberFormat().format(sum_value);   
        })  
    ...
    document.getElementById('delete-modal-btn-del').addEventListener('click',() => {
...
function create_row(line){
    ...
    cell2.innerHTML = currency + Number(line.total).toFixed(2); 
   ...
   row_users.addEventListener('hidden.bs.collapse', function () {
    ...
    const mutationObserverLineUser = new MutationObserver(() =>{   
        ...
        btn_line_user.forEach(user => { 
            ...
            if (line_users_count.innerHTML == document.getElementById('bill_users_total').innerHTML){
    ...
    row.querySelector('#edit_line').addEventListener('click', ()=> line_edit(line.line_id));
    ...
    row.querySelector('#delete_line').addEventListener('click', ()=> {
            ...
            document.getElementById('delete-modal-text').innerHTML = 'Are you sure you want to delete the item?';
            $('#delete-modal').modal('show'); 
    })
    ...
    .then(line => {     
        try{ 
            if (line.error){
                ...
                document.getElementById('id_discount').value = line.discount;  
            }          
    }
    catch(err){      
        show_json_errors(err);
    ...
    .then(line => {
        try{
          if (line.error){
            ...
            row.cells[2].innerHTML = currency + Number(line.total).toFixed(2); 
          }
        }
        catch(err){
            show_json_errors(err);
    ...
    .then(line => {
        try{
          if (line.error){
            ...
            let row_user = document.querySelector(`tr[data-item-users="${line_id}"]`);
        ...
        try{ 
            if (users.error){
                ...
                users.forEach(user => {     
                    ...
                    create_line_user_button(user.user_id, user.name, color,line.line_id);               
                })
            }          
        }
        catch(err){      
    ...
    .then(user => {     
        try{ 
            if (user.error){
                ...
                document.getElementById('user_modal_title').innerHTML = 'Update Person'              
                $('#user_modal').modal('show')  
            }              
        }   
        catch(err){      
    ...
    .then(user => {     
        try{ 
            if (user.error){
                ...
                let person = document.querySelectorAll(`[data-user="${user.user_id}"]`)                
                person.forEach(p => {
        ...
        try{ 
            if (totals.error){
                ...
                totals.users.forEach(total => {                      
                    ...
                    var discount_amount = parseFloat(-1 * total.discount_amount); 
                    var discount_subtotal = parseFloat(total.total_items_cost - total.discount_amount);                                     
                    var tip_value = parseFloat(total.total_items_tip).toFixed(2);                                     
                    var tax_subtotal = parseFloat(discount_subtotal * (1 + (total.tax/100))).toFixed(2);                   
                    var tip_subtotal = parseFloat(parseFloat(tax_subtotal) + parseFloat(tip_value) ).toFixed(2);
                    subtotals.push({"type":'items_subtotal', "value": items_subtotal}, {"type":'discount_amount', "value": discount_amount},
                                   {"type":'discount_subtotal', "value": discount_subtotal}, {"type": "tax_subtotal", "value":tax_subtotal},
                                   {"type": "tip_value", "value": tip_value},{"type": "tip_subtotal", "value": tip_subtotal}
                                    );     
                    ...
                    cost_el.innerHTML = currency + tip_subtotal;
                    ...
                    details_el.classList.add('ms-auto','text-muted','text-decoration-none');
                    ...
                    items_el.addEventListener('show.bs.collapse', function () {                      
                        document.querySelector("#user_details_symbol-"+`${total.user_id}`).innerHTML='-';                          
                    })
                    ...
                    total.line_items.forEach(item => {
                        ...
                        cost_el.innerHTML = currency + item.user_cost;
                        
                        if (item.line_users_names != ''){
                            share_el.innerHTML = item.total + ' split with ' + item.line_users_names;                            
                        }
                        else{
                    ...
                    subtotals.forEach(subtotal =>{
                        ...
                        else if (subtotal.type == 'discount_amount'){
                            name = 'Discount Amount';
                        }
                        else if (subtotal.type == 'discount_subtotal'){
                        ...
                        if (!(subtotal.type == 'tax_subtotal' && total.tax == 0.00 || subtotal.type == 'discount_subtotal' && discount_amount == 0.00 
                             || subtotal.type == 'discount_amount' && discount_amount == 0.00)) {                        
                            ...
                            if ( subtotal.type  == 'items_subtotal'|| subtotal.type == ( 'tip_subtotal') || subtotal.type == ( 'discount_subtotal') ){
                            ...
                            cost_el.innerHTML = currency + parseFloat(subtotal.value).toFixed(2);
                        }    
                ...
                grand_total = parseFloat(((totals.total_lines - totals.user_discount_total)* (1 + (totals.tax/100)))+ total_bill_tip).toFixed(2)
                const formatter = new Intl.NumberFormat('en-US', {
                    minimumFractionDigits: 2,
                  });
                cost_el.innerHTML = currency + formatter.format(grand_total)
                // + Intl.NumberFormat().format(grand_total);
                ...
                totals_details.push({'type':'bill_items_subtotal','value':totals.total_lines},
                                    {"type": 'people_items_subtotal', 'value':totals.user_line_total},
                                    {'type':'unallocated_items', 'value': totals.total_lines-totals.user_line_total},
                                    {"type":'discount_amount', "value": -1 * totals.user_discount_total},
                                    {"type":'discount_subtotal', "value": totals.user_line_total - totals.user_discount_total },
                                    {'type':'tax_subtotal', 'value': (totals.total_lines-totals.user_discount_total)  * (1 + (totals.tax/100))},
                                    {'type':'tip_value', 'value': totals.user_tip_total},
                                    {'type':'tip_subtotal', 'value':  parseFloat((totals.total_lines - totals.user_discount_total)  * (1 + (totals.tax/100))) + parseFloat(totals.user_tip_total)}
                                    );
                ...
                totals_details.forEach(total => {                
                    ...
                    else if (total.type == 'discount_amount') {
                        name = 'Discount Amount'
                    }
                    else if (total.type == 'discount_subtotal') {
                        name = 'Discount subtotal'
                    }
                    ...
                    if (!(total.type == 'tax_subtotal' && totals.tax == 0.00 || total.type == 'discount_subtotal' && totals.user_discount_total == 0.00 
                        || total.type == 'discount_amount' && totals.user_discount_total == 0.00 )) {                
                        ...
                        cost_el = document.createElement('h6');
                        cost_el.classList.add('ms-auto','col-auto');
                        if (total.type == 'unallocated_items' || total.type == 'tip_subtotal' || total.type == 'discount_subtotal'){
                            cost_el.classList.add('border-top', 'border-dark', 'text-dark');
                            name_el.classList.add('text-dark');
                        }    
                        totals_subtotal_el.append(name_el, cost_el); 
                        name_el.innerHTML = name;
                        cost_el.innerHTML = currency + parseFloat(total.value).toFixed(2);
                    }
                })                   
            }          
    }
