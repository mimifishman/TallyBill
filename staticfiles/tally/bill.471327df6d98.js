    ...
    if   (document.getElementById('bill_id')){ 
        ...
        const mutationObserverLineItem = new MutationObserver(() =>{           
            ...
            Array.from(table.rows).forEach((row)=>{
                if (row.cells[2]){
                    sum_value += Number((row.cells[2].innerHTML).replace(String.fromCharCode(36),''));           
                }
            })    
            document.getElementById('total_bill').innerHTML = 'Total: ' + String.fromCharCode(36) + Intl.NumberFormat().format(sum_value);   
        })  
    ...
    .then(line => {
        try{
          if (line.error){
            ...
            row.cells[2].innerHTML = String.fromCharCode(36) + Number(line.total).toFixed(2); 
          }
        }
        catch(err){
            show_json_errors(err);
        ...
        try{ 
            if (totals.error){
                ...
                totals.users.forEach(total => {                      
                    ...
                    var tax_subtotal = parseFloat(items_subtotal * (1 + (total.tax/100))).toFixed(2);                   
                    var tip_subtotal = parseFloat(parseFloat(tax_subtotal) + parseFloat(tip_value) ).toFixed(2);
                    subtotals.push({"type":'items_subtotal', "value": items_subtotal} ,{"type": "tax_subtotal", "value":tax_subtotal},
                    ...
                    subtotals.forEach(subtotal =>{
                        ...
                        if (!(subtotal.type == 'tax_subtotal' && total.tax == 0.00)) {                        
                            ...
                            if ( subtotal.type  == 'items_subtotal'|| subtotal.type == ( 'tip_subtotal') ){
                            ...
                            cost_el.innerHTML = String.fromCharCode(36) + parseFloat(subtotal.value).toFixed(2);
                        }    
                ...
                cost_el.innerHTML = String.fromCharCode(36) + Intl.NumberFormat().format(parseFloat(totals.total_lines *
                                     (1 + (totals.tax/100))+ total_bill_tip ).toFixed(2));
                ...
                totals_details.push({'type':'bill_items_subtotal','value':totals.total_lines},
                                    ...
                                    {'type':'tax_subtotal', 'value': totals.total_lines * (1 + (totals.tax/100))},
                                    {'type':'tip_value', 'value': totals.user_tip_total},
                                    {'type':'tip_subtotal', 'value':  parseFloat(totals.total_lines * (1 + (totals.tax/100))) + parseFloat(totals.user_tip_total) }
                                    );
                ...
                totals_details.forEach(total => {                
                    ...
                    if (!(total.type == 'tax_subtotal' && totals.tax == 0.00)) {                
                        ...
                        cost_el = document.createElement('h6');
                        cost_el.classList.add('ms-auto','col-auto');
                        if (total.type == 'unallocated_items' || total.type == 'tip_subtotal' ){
                            cost_el.classList.add('border-top', 'border-dark', 'text-dark');
                            name_el.classList.add('text-dark');
                        }    
                        totals_subtotal_el.append(name_el, cost_el); 
                        name_el.innerHTML = name;
                        cost_el.innerHTML = String.fromCharCode(36) + parseFloat(total.value).toFixed(2);
                    }
                })                   
            }          
    }
